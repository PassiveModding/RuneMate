...
public enum Herb {
    ...
    public int grimyPrice = -1;
    public int cleanPrice = -1;
    public int unfinishedPrice = -1;
    ...
    public static int getOSBuddyPrice(int id) {
        OSBuddyExchange.GuidePrice gPrice = OSBuddyExchange.getGuidePrice(id);
        if (gPrice != null) {
            return gPrice.getBuying();
        }
    ...
    public static Herb getInfo(String DisplayTxt) {
        for (Herb herb : Herb.values()) {
            if (DisplayTxt.equalsIgnoreCase(herb.displayName()) || DisplayTxt.equalsIgnoreCase(herb.grimy) || DisplayTxt.equalsIgnoreCase(herb.clean) || DisplayTxt.equalsIgnoreCase(herb.unfinishedPot)) {
                return herb;
            }
        }
    ...
    public void setPrices() {
        this.grimyPrice = getGrimyPrice();
        this.cleanPrice = getCleanPrice();
        this.unfinishedPrice = getUnfinishedPrice();
    }

    public int getGrimyPrice() {
        return getOSBuddyPrice(this.grimyId);
    }

    public int getCleanPrice() {
        return getOSBuddyPrice(this.cleanId);
    }

    public int getUnfinishedPrice() {
        //NOTE THIS IS BASED OFF SELLING PRICE, OTHERS ARE BASED OFF BUYING PRICE
        OSBuddyExchange.GuidePrice gPrice = OSBuddyExchange.getGuidePrice(unfinishedId);
        if (gPrice != null) {
            return gPrice.getSelling();
        }
    ...
    public static class Potion {
        ...
        public int secondaryPrice = -1;

        public void setPrices() {
            this.potionPrice = getPotionPrice();
            this.secondaryPrice = getSecondaryPrice();
        }


        public int getSecondaryPrice() {
            return getOSBuddyPrice(this.secondaryId);
        }

        public int getPotionPrice() {
            //NOTE THIS IS BASED OFF SELLING PRICE, OTHERS ARE BASED OFF BUYING PRICE
            OSBuddyExchange.GuidePrice gPrice = OSBuddyExchange.getGuidePrice(potionId);
            if (gPrice != null) {
                return gPrice.getSelling();
            }
